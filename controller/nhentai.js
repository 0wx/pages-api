...
-- https://npmjs.com/packages/nhentai-js

-- I've to put this code in here */

const imageMirrorLink = 'https://image-mirror.herokuapp.com/';

/*
-- this is an image mirror link to prevent isp from blocking the image
-- so I decide to use the copy of the code instead using it's module
        ...
        return new Promise((resolve, reject) => {
            request
                ...
                .then(res => {
                    ...
                    resolve({ title, nativeTitle, details, pages: images.map(img => imageMirrorLink + img), thumbnails: thumbnails.map(img => imageMirrorLink + img), link });
                })
                .catch(reject);
        });
    }
                    ...
                    Object.keys(selector).map(key => {
                        if (parseInt(key)) {
                            ...
                            if ('is' in img.attribs) {
                                bookdetails.thumbnail = imageMirrorLink + img.attribs['data-src'];
                            } else {
                    ...
                    Object.keys(selector).map(key => {
                        if (parseInt(key)) {
                            ...
                            if ('is' in img.attribs) {
                                ...
                                bookdetails.thumbnail = imageMirrorLink + img.attribs['src'].replace(doubleSlashToHttps, 'https://');
                            }
                            bookdetails.title = findObject(book.children, 'name', 'div').children[0].data;
                            details.push(bookdetails);
                        }
